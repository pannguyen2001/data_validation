...
from pipeline.pipeline import data_validation_wrapper
@data_validation_wrapper(
    df=df_learner_account,
    df_processing_config=df_processing_config,
    df_validation_config=df,
    file_path=data_file_path,
...
def process_internal_suspended_list(df) -> pd.DataFrame:
    df = validate_effective_time_logic(df, "Effective from", "Effective to", "%d/%m/%Y")
    return df

process_internal_suspended_list(df_learner_account)
...
#     # ==================== Load common kwargs for replace in processing and validation config ====================
#     common_kwargs: Dict = {
#         "${DATA_FOLDER_PATH}": DATA_FOLDER_PATH,
#         "${EMPTY_LIST}": ["", "nan", np.nan, pd.NA, None, "nan", "NA", "<NA>", "NAT", "null"],
#         "now": datetime.datetime.now()
...
#         key: str = normalize_template_name(file_path)
#         key = "${" + key + "_FILE_PATH}"
#         common_kwargs[key] = file_path


#     # ==================== Load additional validation function ====================
#     additional_fuction: Dict = {

#     }

#     # ==================== Load processing and validation config ====================
...
#     processing_cache: Dict = validator_cache.get(config_type="processing")
#     validation_cache: Dict = validator_cache.get(config_type="validation")
...
#             if sheet_name not in df_validation["sheet_name"].unique():
#                 # logger.info(f"[{sheet_name}] No validation action needs.")
#                 continue
            
#             validation_pipeline.run(
#                 file_path=file_path,
#                 sheet_name=sheet_name,
#                 df_processing_config=df_processing,
#                 df_validation_config=df_validation,
#                 additional_function=additional_fuction.get(sheet_name)
#             )
