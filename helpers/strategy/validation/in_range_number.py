...
class InRangeNumberValidation(ValidationStrategy):
    def __init__(
        ...
        self.kwargs["validation_type"] = "Check in range number"
        self.kwargs["message"] = f"Value should be between {self.kwargs.get('min_value')} and {self.kwargs.get('max_value')}"
