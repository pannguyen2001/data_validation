...
def check_modules(df_check: pd.DataFrame) -> tuple[pd.Series, pd.Series]:
    df_final: pd.DataFrame = pd.DataFrame(columns=["CourseIntakeId", "Missing modules", "Extra modules"])


    # --- Missing modules ---
    missing_modules = df_check["All modules"] - df_check["Course/Product code/Module"]
    missing_mask = missing_modules.map(bool)

    missing_ids = df_check.loc[missing_mask, "index"]
    missing_ids = missing_ids.explode().unique()

    if missing_mask.any():
        df_missing = df_check.loc[missing_mask, ["CourseIntakeId"]].copy()
        df_missing["Missing modules"] = missing_modules[missing_mask]
        df_missing = df_missing.explode("Missing modules", ignore_index=True)
        # df_missing = df_missing.groupby("CourseIntakeId").agg({"Missing modules": list}).reset_index()
        # df_missing["Missing modules"] = df_missing["Missing modules"].map(lambda x: "; ".join(set(x)))
        df_final = df_final.merge(df_missing, how="outer", on=["CourseIntakeId", "Missing modules"])
    #     logger.error(
    #         f"[CourseIntakeFinancialRevenue - Course/Product code/Module] [Special logic] Missing modules: {len(df_missing)} cases. Detail: {df_missing.to_dict(orient='records')}"
    #     )
    # else:
    #     logger.success(
    #         "[CourseIntakeFinancialRevenue - Course/Product code/Module] [Special logic] No missing modules."
    #     )

    # --- Extra modules ---
    extra_modules = df_check["Course/Product code/Module"] - df_check["All modules"]
    extra_mask = extra_modules.map(bool)

    extra_ids = df_check.loc[extra_mask, "index"]
    extra_ids = extra_ids.explode().unique()

    if extra_mask.any():
        df_extra = df_check.loc[extra_mask, ["CourseIntakeId"]].copy()
        df_extra["Extra modules"] = extra_modules[extra_mask]
        df_extra = df_extra.explode("Extra modules", ignore_index=True)
        # df_extra = df_extra.groupby("CourseIntakeId").agg({"Extra modules": list}).reset_index()
        # df_extra["Extra modules"] = df_extra["Extra modules"].map(lambda x: "; ".join(set(x)))
        df_final = df_final.merge(df_extra, how="outer", on=["CourseIntakeId", "Extra modules"])
    #     logger.error(
    #         f"[CourseIntakeFinancialRevenue - Course/Product code/Module] [Special logic] Extra modules: {len(df_extra)} cases. Details: {df_extra.to_dict(orient='records')}"
    #     )
    # else:
    #     logger.success(
    #         "[CourseIntakeFinancialRevenue - Course/Product code/Module] [Special logic] No extra modules."
    #     )
    df_final = df_final.groupby("CourseIntakeId").agg({"Missing modules": list, "Extra modules": list}).reset_index()
    df_final["Missing modules"] = df_final["Missing modules"].map(lambda x: "; ".join(set(x)) if all(pd.notna(x)) else "")
    df_final["Extra modules"] = df_final["Extra modules"].map(lambda x: "; ".join(set(x)) if all(pd.notna(x)) else "")
    if not df_final.empty:
        ...
        # df_final.to_excel(f"{report_folder_path}/{datetime_today}/CourseIntakeFinancialRevenue.xlsx",sheet_name="Missinng or extra modules", index=False)
...
def course_intake_financial_revenue(
    ...
    df_intake_fee_setup: pd.DataFrame = read_reference_data(
        ...
        usecols=["CourseIntakeId", "Method to calculate"] + rate_cols,
    )
    ...
    missing_intake_ids: set = set(df_intake["CourseIntakeId"]) - set(df_check["CourseIntakeId"])
    ...
    missing_intake_fee_setup_ids: set = set(df_intake_fee_setup["CourseIntakeId"]) - set(df_check["CourseIntakeId"])
